...
class ReplayObservationStream(object):
  """Wrapper class for iterating over replay observation knowledge_data.
  ...
  with ReplayObservationStream() as replay_stream:
    ...
    # Get game knowledge_data if needed.
  ...
  def start_replay_from_data(self, replay_data, player_id):
    ...
    try:
      ...
      logging.exception("Error getting replay version from knowledge_data: %s", err)
