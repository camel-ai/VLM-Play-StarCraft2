...
class RunConfig(object):
  ...
  def __init__(self, replay_dir, data_dir, tmp_dir, version,
    ...
    Args:
      ...
      data_dir: Where SC2 should find the knowledge_data and battle.net cache.
  ...
  def map_data(self, map_name, players=None):
    """Return the map knowledge_data for a map by name or path."""
    map_names = [map_name]
    if players:
  ...
  def replay_data(self, replay_path):
    """Return the replay knowledge_data given a path to the replay."""
  ...
  def save_replay(self, replay_data, replay_dir, prefix=None):
    ...
    Args:
      replay_data: The result of controller.save_replay(), ie the binary knowledge_data.
