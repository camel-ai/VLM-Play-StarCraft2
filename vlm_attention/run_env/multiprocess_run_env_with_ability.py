...
from vlm_attention.env.env_core_with_ability import SC2Multimodal_with_ability_Env
from agent.vlm_agent_with_ability import VLMAgentWithAbility
from agent.test_agent_with_ability import TestAgent_With_Ability


"""
multi process run the environment with ability support, one player
"""


map_list = ["ability_map_8marine_3marauder_1medivac_1tank",
            "ability_8stalker_vs_8marine_3marauder_1medivac_tank_map",
            "ability_map_8marine_3marauder_1medivac_1tank_vs_5stalker_2colossis",
            "pvz_task6_level3"]
# Define flags
FLAGS = flags.FLAGS
...
flags.DEFINE_string("agent", "VLMAgentWithAbility", "Agent to use:RandomAgent, VLMAgentWithAbility,TestAgent_With_Ability")
flags.DEFINE_integer("num_processes", 1, "Number of parallel processes to use")
...
def run_episode(config: dict) -> dict:
    ...
    try:
        ...
        env = SC2Multimodal_with_ability_Env(**env_config)

        # 创建智能体
        agent_class = {
            "TestAgent_With_Ability": TestAgent_With_Ability,
            "VLMAgentWithAbility":VLMAgentWithAbility
        }[agent_name]
